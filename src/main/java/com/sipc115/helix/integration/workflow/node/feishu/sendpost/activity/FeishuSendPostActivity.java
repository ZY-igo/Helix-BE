 ...
 * <h3>幂等性保护：</h3>
 * <p>
 * 所有 sendPost 方法都包含 executionId、nodeId 和 retryCount 参数，
 * 用于实现幂等性检查。实现类会通过 WorkflowTraceService 查询
 * 之前是否已经成功执行过相同的节点，如果已成功则直接返回缓存结果。
 *
 * @author Helix Team
 * @since 2.0.0
...
public interface FeishuSendPostActivity {
     ...
     * <h3>主要流程：</h3>
     * <ol>
     *   <li>检查该节点是否已成功执行过（通过 executionId + nodeId）</li>
     ...
     */
    String sendPost(Long executionId, Integer retryCount, String nodeId,
     ...
     * @return 发送成功后的消息 ID
     */
    String sendPostWithConfig(Long executionId, Integer retryCount, String nodeId,
